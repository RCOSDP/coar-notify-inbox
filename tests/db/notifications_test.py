...
@patch('db.notifications.get_adapter')
@pytest.mark.asyncio
async def test_get_notifications_paged(mock_get_adapter):
    adapter = AsyncMock()
    adapter.find.return_value = []
    mock_get_adapter.return_value = adapter

    await get_notifications(page=3, page_size=10)

    adapter.find.assert_called_once_with(
        NOTIFICATIONS_COLLECTION_NAME, {}, sort=("updated", -1), skip=20, limit=10
    )
